...
const CALISTHENICS_PROGRESSION_RULES = [
  ...
  "  * ASSISTED TARGET: band/spotter/foot-on-wall assistance through the FULL target skill. Use when the athlete cannot yet own any phase of the unassisted skill.",
  ...
  "  * Prescribing 'Wall 90-Degree HSPU' as a progression for 'Freestanding 90-Degree HSPU'. The wall variant is a different skill (chest-to-wall mechanics, different shoulder angle); it is NOT a rung of the freestanding ladder. Legal bridges for freestanding 90-deg HSPU are: eccentric freestanding 90-deg HSPU, isometric freestanding 90-deg hold, partial-ROM freestanding HSPU at the 90-deg window, and assisted freestanding HSPU (foot on wall ONLY for balance, not for load).",
  ...
  "    Wall (chest-to-wall) family, simpler to harder: Pike Push-up -> Elevated Pike -> Wall Handstand Hold -> Wall HSPU partial -> Wall HSPU full ROM -> Deficit Wall HSPU -> 90-deg Wall HSPU.",
  "    Freestanding family, simpler to harder: Freestanding Handstand Hold -> Freestanding HSPU negative -> Freestanding HSPU partial -> Freestanding HSPU full ROM -> Deficit Freestanding HSPU -> 90-deg Freestanding HSPU.",
  "    Bridging BETWEEN families (wall -> freestanding) requires a FREESTANDING balance prerequisite (60+ s freestanding hold), not a strength substitution.",
  "",
  ...
  "",
  "  Planche family: Planche Lean -> Tuck -> Advanced Tuck -> Straddle -> Half-lay -> Full. (Holds and presses are different skill families even within planche.)",
  "",
  "  Front Lever family: Tuck -> Advanced Tuck -> Single-leg -> Straddle -> Half-lay -> Full.",
  "",
  "  Muscle-up family: Strict Chest-to-bar Pull-up -> Explosive Hip-to-bar -> False-grip Hold -> Banded MU -> MU Negative -> Strict MU.",
  "",
  ...
  "  - Prescribing a wall variant when the goal lives in the freestanding family (or vice versa).",
...
const MULTI_WEEK_BLOCK_RULES = [
  ...
  "",
  "  DAILY UNDULATING PERIODISATION (DUP, intermediate default): the heavy/moderate/light day character within the week is preserved. Week-to-week, progress each day type independently. For ACCUMULATION: heavy day add 1 set OR +2.5 kg, moderate day add 1-2 reps, light day add 1 set OR new variation. Week 4 is a DELOAD (volume -40 to -50%, loads -10%, RIR +1 to +2).",
  "",
  "  CONJUGATE (advanced strength): ME exercise ROTATES each week (a four-variation rotation is standard, so each ME lift appears once across the block). DE waves intensity (W1=50%, W2=55%, W3=60% of competition lift, W4 deload 45%). RE work waves volume (W1 baseline, W2 +1 set, W3 +1 more set, W4 halved).",
  "",
  "  BLOCK PERIODISATION (multi-quality, hybrid athletes): allocate the 4 weeks to qualities. Common 4-week split for a strength + grappling athlete: W1-2 emphasise the qualified weakness (e.g. max strength), maintain the other (sport work + 1-2 conditioning rows). W3 SHIFTS emphasis to the second quality (conditioning / sport-specific work capacity) while maintaining strength via minimum effective dose. W4 is a DELOAD or a transition micro-peak.",
  "",
  "  HIGH/LOW (masters / recovery-limited): the High/Low split is STRUCTURAL and constant across all 4 weeks. Within that constant structure, progress only the HIGH-day intensity OR the LOW-day duration, never both at once. W4 cuts one HIGH day to a LOW day for recovery.",
  "",
  ...
  "  - Week 4 showing the same or higher volume/intensity than Week 3 when the block phase is ACCUMULATION or INTENSIFICATION (a deload was due).",
...
const WARMUP_RULES = [
  ...
  "",
  "PART C - OUTPUT FORMAT IN TSV:",
  "Inside each Day group, the rows must go in this order:",
  "  1. General prep rows (3 rows: pulse-raiser + 2 activation/mobility)",
  "  2. Specific ramp rows for the main lift (variable number, depending on W%)",
  "  3. Working sets of the main lift",
  "  4. Accessory work",
  "",
  "Mark warm-up rows by putting '[WARMUP]' as a PREFIX on the Exercise cell. Example: '[WARMUP] Bike, easy'. '[WARMUP] Ankle dorsiflexion drill, 2x8/side'. '[WARMUP] Back squat ramp set, 70 kg'. The Sets/Reps/Rest/Target RPE cells get filled normally; the Weight cell gets the ramp kg for ramp rows or 'BW' for bodyweight prep rows.",
  "",
  "If the day has TWO main lifts (e.g. squat + bench on the same day), emit a separate specific ramp for the SECOND main lift directly before its first working set. Do not re-do general prep between the two.",
  "",
  ...
  "",
  "HARD FAILS (validator will flag):",
  "  - A training day with a main barbell lift and no specific ramp rows above the working sets.",
  "  - A pure-bodyweight skill day (HSPU/OAP/planche/lever) with no skill-graded primer row.",
