...
function rateKey(req, group) { return `${group}:${clientIp(req)}`; }
...
function validConsent(intake) {
  ...
  return Boolean(c && c.health_data === true && typeof c.policy_version === "string" && c.policy_version.length > 0 && typeof c.consented_at === "string" && !Number.isNaN(Date.parse(c.consented_at)));
}
...
async function requireActivePassForToken(token) {
  ...
  if (pass.status !== "active" || passExpired(pass)) return { error: "This Program Pass has expired. Purchase a new Program Pass to continue.", status: 403 };
  return { pass };
}
  ...
  try {
    ...
    if (req.method === "POST" && req.path === "/api/build") {
      ...
      if (token) {
        ...
        if (active.pass.initial_build_completed_at) return res.status(403).json({ error: "This Program Pass has already created its training block. Use Adjust for changes, or purchase a new Program Pass for a completely new block." });
        return next();
      }
    ...
    if (req.method === "GET" && programMatch) {
      const active = await requireActivePassForToken(programMatch[1]);
      if (active.error) return res.status(active.status).json({ error: active.error });
      return next();
    }
    ...
    if (req.method === "POST" && (req.path === "/api/adjust" || req.path === "/api/set-language")) {
      ...
      if (req.path === "/api/adjust") {
        ...
        if (used >= limit) return res.status(403).json({ error: `You've used all ${limit} included Program Pass adjustments. Purchase a new Program Pass for a new training block.` });
      }
      return next();
    }
    next();
...
function securityMiddleware(req, res, next) {
  ...
  if (bodyToken && !TOKEN_RE.test(bodyToken)) return res.status(400).json({ error: "Invalid personal code." });
  const programMatch = req.path.match(/^\/api\/program\/([^/]+)$/);
  if (programMatch && !TOKEN_RE.test(programMatch[1])) return res.status(400).json({ error: "Invalid personal code." });
  const jobMatch = req.path.match(/^\/api\/job\/([^/]+)$/);
  if (jobMatch && !JOB_RE.test(jobMatch[1])) return res.status(400).json({ error: "Invalid job id." });
  ...
  if (req.method === "POST" && req.path === "/api/build") {
    ...
    if (!serialized || serialized.length > MAX_INTAKE_CHARS || tooLarge(intake)) return res.status(413).json({ error: "The intake contains too much text. Please shorten the entries and try again." });
...
async function runRetentionCleanup() {
  await privacyStore.purgeExpiredOperationalData();
  if (!PROGRAM_PASS_ENFORCEMENT) return;
  const graceMs = Math.max(0, PROGRAM_PASS_DATA_GRACE_DAYS) * 24 * 60 * 60 * 1000;
  const expired = await passStore.expiredTokens(Date.now() - graceMs);
  for (const token of expired) {
    try { await privacyStore.deleteClientData(token); }
    catch (e) { console.warn("expired coaching-data deletion failed:", e && e.message); }
  }
}
...
express.application.listen = function (...args) {
  if (!this.__privacyRoutesInstalled) {
    ...
    this.post("/api/program-pass-status", async (req, res) => {
      try {
        ...
        return res.json({ status: pass.status, expires_at: pass.expires_at, initial_build_completed: Boolean(pass.initial_build_completed_at), adjustments_used: used, adjustments_remaining: Math.max(0, limit - used), adjustment_limit: limit });
      } catch (e) {
    ...
    this.post("/api/analytics-event", async (req, res) => {
      try {
        ...
        if (!isAllowedAnalyticsEvent(event)) return res.status(400).json({ error: "Invalid analytics event." });
    ...
    this.get("/api/admin/analytics-summary", async (req, res) => {
      try {
        if (!ADMIN_PROVISION_KEY || req.get("x-admin-provision-key") !== ADMIN_PROVISION_KEY) return res.status(404).json({ error: "Not found." });
        const days = Math.max(1, Math.min(365, Number(req.query?.days || 30)));
        return res.json({ days, rows: await analyticsStore.summary(days) });
      } catch (e) {
  ...
  runRetentionCleanup().catch((e) => console.warn("retention cleanup failed:", e && e.message));
