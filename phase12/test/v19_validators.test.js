...
// ---------------------------------------------------------------------------
// TEST 6: sport scheduling is context-aware, not a universal 48h ban.
// ---------------------------------------------------------------------------
test("validateSportDayCoupling: adjacent hard sport is advisory; explicit <4h same-day collision fails", () => {
  const adjacentIntake = {
    sport: "BJJ",
    sport_schedule: [{ day: "Sat", intensity: "hard" }, { day: "Sun", intensity: "hard" }],
  };
  const fridayLower = program([
    "Fri\tBack Squat\t180 kg\t3\t3\t180s\tRIR 2\t\t",
    "Fri\tRomanian Deadlift\t140 kg\t3\t6\t120s\tRIR 2\t\t",
  ]);
  const review = validateSportDayCoupling(fridayLower, adjacentIntake);
  assert.equal(review.ok, true);
  assert.ok(review.warnings.some((v) => v.type === "lower_before_hard_sport"));

  const shortGapIntake = {
    sport: "BJJ",
    sport_schedule: [{ day: "Wed", intensity: "hard", gap_hours: 2 }],
  };
  const sameDayLower = program([
    "Wed\tBack Squat\t180 kg\t3\t3\t180s\tRIR 2\t\t",
    "Wed\tRomanian Deadlift\t140 kg\t3\t6\t120s\tRIR 2\t\t",
  ]);
  assert.throws(
    () => validateSportDayCoupling(sameDayLower, shortGapIntake),
    (err) => {
      assert.ok(err instanceof RetriableValidationError);
      assert.equal(err.code, "SPORT_DAY_COUPLING_VIOLATION");
      assert.ok(err.details.violations.some((v) => v.type === "hard_sport_short_gap_lower"));
      return true;
    }
  );

  const spacedSameDay = { sport: "BJJ", sport_schedule: [{ day: "Wed", intensity: "hard", gap_hours: 6 }] };
  const spaced = validateSportDayCoupling(sameDayLower, spacedSameDay);
  assert.equal(spaced.ok, true);
  assert.ok(spaced.warnings.some((v) => v.type === "hard_sport_same_day_lower"));
});
...
test("validateExercisesAgainstDictionary rejects fabricated 90-degree wall HSPU even if phrased cleanly", () => {
  const prog = program([
    "Mon\t90-Degree Wall Handstand Push-up\tBW\t3\t2\t3 min\tRIR 1\tskill\t",
  ]);
  assert.throws(
    () => validateExercisesAgainstDictionary(prog, {}),
    (err) => err && err.code === "EXERCISE_HALLUCINATION"
  );
});

test("validateExercisesAgainstDictionary rejects wall-supported Human Flag inventions", () => {
  const prog = program([
    "Mon\tWall-Supported Tuck Human Flag\tBW\t3\t5s\t2 min\tRIR 1\tskill\t",
  ]);
  assert.throws(
    () => validateExercisesAgainstDictionary(prog, {}),
    (err) => err && err.code === "EXERCISE_HALLUCINATION"
...
// ---------------------------------------------------------------------------
// SPORT CALENDAR: chosen gym days must respect athlete availability and count.
// ---------------------------------------------------------------------------
test("validateSportDayCoupling: gym day availability and requested session count are hard constraints", () => {
  const intake = {
    sport: "BJJ",
    sport_schedule: [{ day: "Wed", intensity: "hard" }],
    days_per_week: 2,
    available_gym_days: ["Mon", "Fri", "Sat"],
  };
  const badDay = program([
    "Mon\tBack Squat\t150 kg\t3\t5\t180s\tRIR 2\t\t",
    "Thu\tStrict Overhead Press\t60 kg\t3\t5\t180s\tRIR 2\t\t",
  ]);
  assert.throws(
    () => validateSportDayCoupling(badDay, intake),
    (err) => {
      assert.equal(err.code, "SPORT_DAY_COUPLING_VIOLATION");
      assert.ok(err.details.violations.some((v) => v.type === "gym_day_unavailable"));
      return true;
    }
  );

  const wrongCount = program([
    "Mon\tBack Squat\t150 kg\t3\t5\t180s\tRIR 2\t\t",
  ]);
  assert.throws(
    () => validateSportDayCoupling(wrongCount, intake),
    (err) => {
      assert.ok(err.details.violations.some((v) => v.type === "gym_day_count_mismatch"));
      return true;
    }
  );
  ...
  const good = program([
    "Mon\tBack Squat\t150 kg\t3\t5\t180s\tRIR 2\t\t",
    "Fri\tStrict Overhead Press\t60 kg\t3\t5\t180s\tRIR 2\t\t",
  ]);
