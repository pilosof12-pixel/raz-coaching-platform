...
export function validateLaunchIntake(intake){
  ...
  if(!Array.isArray(intake.primary_goals) || intake.primary_goals.length < 1) return "Please list at least one primary goal.";
  if(intake.primary_goals.length > 10 || intake.primary_goals.some(g=>!nonEmptyString(g))) return "Primary goals must be a short list of specific goals.";
