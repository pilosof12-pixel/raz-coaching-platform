...
once(
  ...
  'const GEMINI_MODEL = process.env.GEMINI_MODEL || "gemini-2.5-flash";\nconst OPENAI_API_KEY = process.env.OPENAI_API_KEY || "";\nconst OPENAI_MODEL = process.env.OPENAI_MODEL || "gpt-5.4";\nconst OPENAI_REASONING_EFFORT = process.env.OPENAI_REASONING_EFFORT || "medium";\nconst OPENAI_MAX_OUTPUT_TOKENS = Number(process.env.OPENAI_MAX_OUTPUT_TOKENS || 18000);\nconst AI_REQUEST_TIMEOUT_MS = Number(process.env.AI_REQUEST_TIMEOUT_MS || (OPENAI_API_KEY ? 180000 : 110000));\nconst BUILD_JOB_TIMEOUT_MS = Number(process.env.BUILD_JOB_TIMEOUT_MS || (OPENAI_API_KEY ? 240000 : 210000));\nlet lastAIUsage = null;',
  "provider config"
);

const openAIProvider = `const OPENAI_COMPACT_DEVELOPER = [\n  "You are the RAZ Performance coaching engine. Build an advanced, evidence-aware, sport-aware strength program from the supplied intake and rule packet.",\n  "The user message already contains the intake, programming rules, Phase 15 quality gate, localization rules, and client output contract. Treat those as mandatory. Do not ask questions and do not use profile memory outside the supplied intake.",\n  "Prioritize the athlete's top two goals, current demonstrated performance, sport schedule, pain tolerance, session-time cap, and recovery constraints. Secondary goals need meaningful dose when the supplied rules require it. Avoid junk volume and unrequested hard conditioning.",\n  "STRUCTURE IS A HARD CONTRACT: emit exactly one START_WEEK1_TSV ... END_WEEK1_TSV block containing ALL Week 1 gym days, exactly one corresponding block for each later week requested by the contract, and never restart START_WEEK1_TSV for individual days. Each TSV row must have exactly the contract columns.",\n  "Do not emit [REVIEW], internal validator labels, support messages, or unresolved placeholder exercise names. Prefer canonical/common exercise names that will survive deterministic exercise validation. Warm-up work must use the literal [WARMUP] prefix only when required by the contract.",\n  "Before answering, internally verify: dual-outcome primary goals each have specific exposure; advanced skill work matches current level; OHP/secondary dose is real; power precedes heavy strength; Zone 2 does not become hard intervals; pain-risk exercises are tolerance-gated; all requested training days are inside one Week 1 block; narrative matches the final table.",\n  "Return only the final client-ready program in the exact output format requested by the user message."\n].join("\\n");\n\nasync function runEngineRaw(userContent) {\n  if (OPENAI_API_KEY) {\n    const controller = new AbortController();\n    const timer = setTimeout(() => controller.abort(), AI_REQUEST_TIMEOUT_MS);\n    const started = Date.now();\n    try {\n      const developerChars = OPENAI_COMPACT_DEVELOPER.length;\n      const userChars = String(userContent || "").length;\n      if (developerChars > 100000) throw new Error("Compact OpenAI developer prompt unexpectedly exceeded 100000 characters.");\n      console.log("OpenAI compact prompt layout:", JSON.stringify({ developer_chars: developerChars, user_chars: userChars, legacy_engine_chars_not_sent: ENGINE.length }));\n      const r = await fetch("https://api.openai.com/v1/responses", {\n        method: "POST",\n        headers: {\n          "Authorization": "Bearer " + OPENAI_API_KEY,\n          "Content-Type": "application/json"\n        },\n        body: JSON.stringify({\n          model: OPENAI_MODEL,\n          input: [\n            { role: "developer", content: OPENAI_COMPACT_DEVELOPER },\n            { role: "user", content: userContent }\n          ],\n          reasoning: { effort: OPENAI_REASONING_EFFORT },\n          max_output_tokens: OPENAI_MAX_OUTPUT_TOKENS\n        }),\n        signal: controller.signal\n      });\n      const data = await r.json().catch(() => ({}));\n      if (!r.ok) {\n        const msg = data?.error?.message || ("OpenAI HTTP " + r.status);\n        throw new Error(msg);\n      }\n      const text = (data.output || [])\n        .flatMap(item => item?.content || [])\n        .filter(c => c?.type === "output_text" && typeof c.text === "string")\n        .map(c => c.text)\n        .join("");\n      const u = data.usage || {};\n      const input = Number(u.input_tokens || 0);\n      const output = Number(u.output_tokens || 0);\n      const cached = Number(u.input_tokens_details?.cached_tokens || 0);\n      const reasoning = Number(u.output_tokens_details?.reasoning_tokens || 0);\n      const longContext = input > 272000;\n      const inputRate = longContext ? 5.0 : 2.5;\n      const cachedRate = longContext ? 0.5 : 0.25;\n      const outputRate = longContext ? 22.5 : 15.0;\n      const estimatedCostUsd = ((Math.max(0, input - cached) * inputRate) + (cached * cachedRate) + (output * outputRate)) / 1000000;\n      lastAIUsage = {\n        provider: "openai", model: OPENAI_MODEL, reasoning_effort: OPENAI_REASONING_EFFORT,\n        input_tokens: input, cached_input_tokens: cached, output_tokens: output, reasoning_tokens: reasoning,\n        compact_developer_chars: developerChars, user_prompt_chars: userChars, legacy_engine_chars_not_sent: ENGINE.length,\n        long_context_pricing: longContext, estimated_cost_usd: Number(estimatedCostUsd.toFixed(4)),\n        elapsed_ms: Date.now() - started\n      };\n      console.log("OpenAI generation usage:", JSON.stringify(lastAIUsage));\n      if (!text) throw new Error("OpenAI returned no output_text content.");\n      return text;\n    } finally {\n      clearTimeout(timer);\n    }\n  }\n  if (USE_PPLX_PROXY) {`;
...
console.log("built server.phase15.js with compact OpenAI rule-packet execution path + Phase 15 QA");
