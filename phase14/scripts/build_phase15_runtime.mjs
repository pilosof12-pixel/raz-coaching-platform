...
// The generated runtime adds OpenAI as the preferred provider when OPENAI_API_KEY
// exists, keeps Gemini available when it does not, and injects the Phase 15 QA gate.
...
// Provider config. OPENAI_API_KEY wins only when explicitly present. Gemini remains
// untouched as a fallback provider for deployments that do not have an OpenAI key.
once(
...
// One paid OpenAI attempt for the initial A/B test. Existing Gemini behavior keeps
// its three-attempt recovery loop. This prevents a bad QA cycle from consuming the
// user's small test budget several times in one build.
once(
...
// Safe deterministic repair first, then substantive QA. We only reorder power work;
// we never invent exercises, loads or volume to make a benchmark pass.
once(
...
console.log("built server.phase15.js with OpenAI/Gemini provider selection + Phase 15 QA");
