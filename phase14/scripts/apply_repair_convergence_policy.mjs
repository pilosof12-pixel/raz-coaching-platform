...
// The event progression validator accepts an explicitly identified measurable
// marathon long run as the progression-bearing event session. When that is the
// only remaining defect, repair the role label/dose progression without adding
// extra run days, intervals or cross-training to a high-concurrency athlete.
...
if (!src.includes(eventMarker)) {
  ...
  const count = src.split(anchor).length - 1;
  if (count !== 1) throw new Error(`Marathon event repair anchor expected once, found ${count}`);
...
if (!src.includes(convergenceMarker) || !src.includes(eventMarker) || !src.includes(promptBudgetMarker)) {
