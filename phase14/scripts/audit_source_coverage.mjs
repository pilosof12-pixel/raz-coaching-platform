...
const RULES = [
  ...
  { id: 'A85.2', rule: 'Pulling volume capped at 6 working sets when mat hours >= 10', told: /pulling[^.]{0,40}(cap|6 (working )?sets)/i, checks: /matHours|pullingCap|MAT_HOURS/ },
  { id: 'A85.3', rule: 'Grip fatigue >= 7 forces the >=10 mat-hour protocol', told: /grip[_ ]fatigue/i, checks: /grip_?fatigue|gripFatigue/i },
  { id: 'A85.4', rule: 'Competition week: no strength above 70% 1RM', told: /70%[^.]{0,30}1RM/i, checks: /SEVENTY|0\.70[^\d]|70\s*\/\s*100/ },
  { id: 'A85.5', rule: 'Neck pain >= 3/10 locks out axial loading', told: /neck pain/i, checks: /neckPain|axialLock|AXIAL_/i },
  // --- Article 86, striking --------------------------------------------------
  ...
  { id: 'A86.5', rule: 'Concussion or head impact generates zero gym work', told: /concussion|head impact/i, checks: /concussion|headImpact/i },
  { id: 'A86.6', rule: 'Overhead pressing eliminated from T-3 weeks to fight week', told: /overhead[^.]{0,60}(T-3|fight week)/i, checks: /overheadLockout|OVERHEAD_[A-Z]*LOCK/i },
  // --- Article 87, MMA -------------------------------------------------------
  ...
  { id: 'A88.4', rule: 'Tendon pain >= 3/10 removes eccentric and plyo, substitutes isometrics', told: /tendon pain/i, checks: /tendonPain|isometricSub/i },
  ...
  { id: 'A89.5', rule: 'Minimum 2 complete recovery days per 7-day microcycle', told: /recovery day/i, checks: /completeRecoveryDays|minRecoveryDays|REST_DAYS_BELOW/ },
  ...
  { id: 'TAP.1', rule: 'Taper reduces volume by 41-60% of pretaper', told: /41[^.]{0,20}60/i, checks: /taperVolumeBand|PRETAPER|0\.4[0-9][^\d].{0,40}0\.6/ },
  { id: 'TAP.2', rule: 'Taper duration 8-14 days is the default window', told: /8 to 14|8-14/i, checks: /taperDays|TAPER_WINDOW|8\s*,\s*14/ },
  { id: 'TAP.3', rule: 'Frequency held at 30-50% of pretaper, not cut with volume', told: /30 to 50|30-50/i, checks: /taperFrequency|FREQUENCY_HELD/i },
