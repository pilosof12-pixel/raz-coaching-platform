...
if (!src.includes(feedbackMarker)) {
  ...
  // end points at the newline immediately before the old function's closing brace.
  // The replacement already contains its own closing brace, so consume the old
  // close plus one newline before preserving the following async function.
  const tailStart = end + '\n}\n'.length;
