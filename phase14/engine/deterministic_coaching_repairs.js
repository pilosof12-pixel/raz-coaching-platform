...
function transformWeekBlocks(program, transformRows) {
  ...
  for (const line of lines) {
    if (/^START_WEEK\d+_TSV\s*$/.test(line)) { inWeek = true; out.push(line); continue; }
    if (/^END_WEEK\d+_TSV\s*$/.test(line)) { flush(); inWeek = false; out.push(line); continue; }
    if (!inWeek) { out.push(line); continue; }
    if (!header && line.trim()) { header = line; delimiter = line.includes('\t') ? '\t' : ','; continue; }
...
export function repairYouthPrimarySkillOrder(program, intake = {}) {
  ...
  return transformWeekBlocks(program, (rows, delimiter) => {
    ...
    let group = [];
    let currentDay = null;
    const pushGroup = () => { if (group.length) groups.push(group); group = []; };
    for (const row of parsed) {
      if (!row.line.trim()) { pushGroup(); groups.push([row]); currentDay = null; continue; }
    ...
    return repaired.map((row) => row.line);
  });
}

export function repairHybridMarathonEventAnchor(program, intake = {}) {
  if (!/\bmarathon\b/i.test(goals(intake))) return String(program || '');
  ...
  return transformWeekBlocks(program, (rows, delimiter) => {
    ...
    const measurable = runRows.find((row) => /\b\d+(?:\.\d+)?\s*(?:km|miles?|min|minutes?|hours?|hrs?)\b/i.test(`${row.cells[WEIGHT] || ''} ${row.cells[REPS] || ''} ${row.cells[NOTES] || ''}`));
