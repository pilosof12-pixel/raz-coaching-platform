...
function makeSupabaseStorage() {
  ...
  function isTransientSupabaseFailure(err) {
    ...
    return err instanceof TypeError || msg.includes("fetch failed") || msg.includes("network") || msg.includes("socket") || msg.includes("econnreset") || msg.includes("etimedout");
  }

  async function withSupabaseRetry(operation, fn, attempts = 3) {
    let lastErr;
    for (let attempt = 1; attempt <= attempts; attempt += 1) {
      try {
        ...
        lastErr = err;
        if (!isTransientSupabaseFailure(err) || attempt >= attempts) break;
        const delay = attempt === 1 ? 350 : 900;
        console.warn(`[supabase] transient ${operation} failure; retry ${attempt}/${attempts - 1}:`, err?.message || err);
        await new Promise((resolve) => setTimeout(resolve, delay));
      }
    }
    const wrapped = new Error(`Supabase ${operation} failed after retry: ${lastErr?.message || lastErr || "unknown error"}`);
  ...
  return {
    ...
    async createJob(id, token, kind, now) {
      const s = await sb();
      const result = await withSupabaseRetry("createJob/insert", () =>
        s.from("jobs").insert({
          id, token, kind, status: "pending", stage: "queued", attempt: 0, detail: null,
          program: null, error: null, created_at: now, updated_at: now,
        })
      );
      assertSupabase(result, "createJob/insert");
    },
    async updateJobProgress(id, stage, attempt = 0, detail = null, now = Date.now()) {
      const s = await sb();
      const result = await withSupabaseRetry("updateJobProgress/update", () =>
        s.from("jobs").update({ stage, attempt, detail, updated_at: now }).eq("id", id)
      );
      assertSupabase(result, "updateJobProgress/update");
    },
    ...
    async getJob(id) {
      const s = await sb();
      const result = await withSupabaseRetry("getJob/select", () =>
        s.from("jobs").select("*").eq("id", id).maybeSingle()
      );
