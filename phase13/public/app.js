...
(function () {
  ...
  const LS_INTAKE_DRAFT = "raz_intake_draft_v1";

  // Prefer browser storage so tokens and intake drafts survive refreshes/errors.
  // Fall back to memory in restricted preview/iframe environments.
  const memStore = {};
  const store = {
    get(k){
    ...
    set(k,v){
      memStore[k] = String(v);
      try { window.localStorage.setItem(k, String(v)); } catch (_) {}
    },
    remove(k){
      delete memStore[k];
  ...
  function buildSportDays() {
    ...
    DAYS.forEach((day) => {
      ...
      ["Light", "Moderate", "Hard"].forEach((lvl) => {
        ...
        b.type = "button";
        b.className = "ibtn";
        b.dataset.level = lvl.toLowerCase();
        b.textContent = lvl;
  ...
  function bindChoice(groupId, hiddenId, panelId, showWhen = "yes") {
    const group = $(groupId);
    const hidden = $(hiddenId);
    const panel = panelId ? $(panelId) : null;
    if (!group || !hidden) return;
    const syncPanel = () => {
      if (panel) panel.classList.toggle("hidden", hidden.value !== showWhen);
    };
  ...
  bindChoice("sport-choice", "has_sport", "sport-details", "yes");
  bindChoice("pain-choice", "has_pain", "pain-details", "yes");
  bindChoice("mobility-choice", "has_mobility_limit", "mobility-details", "yes");
  // Unlike the yes/no panels above, the gym-day selector opens on `limited`.
  bindChoice("gym-availability-choice", "gym_availability_mode", "gym-availability-details", "limited");
  ...
  // A failed build or page refresh should never force the athlete to re-enter a long intake.
  let draftSaveTimer = null;

  function snapshotDraft() {
    const fields = {};
    document.querySelectorAll("#intake-card input[id], #intake-card select[id], #intake-card textarea[id]").forEach((el) => {
    ...
    return {
      ...
      gym_availability: { ...gymAvailability },
      performance,
    };
  }

  function saveDraftNow() {
    try { store.set(LS_INTAKE_DRAFT, JSON.stringify(snapshotDraft())); } catch (_) {}
  }
  ...
  function syncChoiceUI(groupId, hiddenId, panelId, showWhen = "yes") {
    const group = $(groupId);
    const hidden = $(hiddenId);
    if (group && hidden) {
      group.querySelectorAll(".choice-btn").forEach((btn) => {
    ...
    const panel = panelId ? $(panelId) : null;
    if (panel && hidden) panel.classList.toggle("hidden", hidden.value !== showWhen);
  }
  ...
  function restoreDraft() {
    ...
    try {
      ...
      if (!draft || draft.version !== 1) return 0;
      Object.entries(draft.fields || {}).forEach(([id, value]) => {
        const el = $(id);
        if (el) el.value = value ?? "";
      });

      Object.keys(sportState).forEach((day) => { sportState[day] = null; });
      Object.entries(draft.sport_state || {}).forEach(([day, value]) => {
        if (Object.prototype.hasOwnProperty.call(sportState, day)) sportState[day] = value || null;
      });
      document.querySelectorAll("#sport-days .sport-day").forEach((row) => {
        const dayBtn = row.querySelector(".daybtn");
        const day = dayBtn?.dataset.day;
        const value = day ? sportState[day] : null;
        dayBtn?.classList.toggle("on", !!value);
        const intensity = row.querySelector(".intensity");
        intensity?.classList.toggle("hidden", !value);
        row.querySelectorAll(".ibtn").forEach((b) => b.classList.toggle("on", b.dataset.level === value));
      });

      Object.keys(gymAvailability).forEach((day) => { gymAvailability[day] = false; });
      Object.entries(draft.gym_availability || {}).forEach(([day, value]) => {
        if (Object.prototype.hasOwnProperty.call(gymAvailability, day)) gymAvailability[day] = !!value;
      });
      document.querySelectorAll("#gym-available-days .daybtn").forEach((btn) => {
        btn.classList.toggle("on", !!gymAvailability[btn.dataset.day]);
      });

      const perfHost = $("performance-rows");
      if (perfHost && Array.isArray(draft.performance) && draft.performance.length) {
        perfHost.innerHTML = "";
        performanceRowCount = 0;
        draft.performance.slice(0, 8).forEach((r) => addPerformanceRow(r.name || "", r.result || ""));
      }

      syncGoalHierarchy();
      syncLegacyFields();
      syncChoiceUI("sport-choice", "has_sport", "sport-details", "yes");
      syncChoiceUI("pain-choice", "has_pain", "pain-details", "yes");
      syncChoiceUI("mobility-choice", "has_mobility_limit", "mobility-details", "yes");
      syncChoiceUI("gym-availability-choice", "gym_availability_mode", "gym-availability-details", "limited");
      if (eqType) $("equipment_specify_wrap").classList.toggle("hidden", eqType.value !== "specify");
      intakeLangCtl.set(($("language") && $("language").value) || "en");
      return Number.isInteger(draft.wizard_step) ? draft.wizard_step : 0;
    } catch (_) {
      return 0;
    }
  }

  document.addEventListener("input", (e) => { if (e.target.closest?.("#intake-card")) scheduleDraftSave(); });
  document.addEventListener("change", (e) => { if (e.target.closest?.("#intake-card")) scheduleDraftSave(); });
  document.addEventListener("click", (e) => { if (e.target.closest?.("#intake-card button")) setTimeout(scheduleDraftSave, 0); });
  ...
  const restoredWizardStep = restoreDraft();
  showWizardStep(restoredWizardStep);
