...
function makeSupabaseEntitlements() {
  ...
  return {
    ...
    async incrementAdjustment(token) {
      ...
      const { error } = await s.from("program_passes").update({ adjustment_count: next }).eq("code", pass.code);
      if (error) throw error;
...
async function makeSqliteEntitlements() {
  ...
  if (!cols.includes("adjustment_count")) db.exec("ALTER TABLE program_passes ADD COLUMN adjustment_count INTEGER NOT NULL DEFAULT 0");
  if (!cols.includes("initial_build_completed_at")) db.exec("ALTER TABLE program_passes ADD COLUMN initial_build_completed_at INTEGER");

  return {
    backend: "sqlite",
    async issuePass(code, now, adjustmentLimit = 6) {
      db.prepare("INSERT INTO program_passes (code,status,activated_token,created_at,activated_at,expires_at,adjustment_limit,adjustment_count,initial_build_completed_at) VALUES (?,'issued',NULL,?,NULL,NULL,?,0,NULL)").run(code, now, adjustmentLimit);
      return this.getPass(code);
    },
    ...
    async activatePass(code, token, now, accessDays = 56) {
      ...
      if (pass.activated_token && pass.activated_token !== token) return pass;
      const expiresAt = pass.expires_at || (now + accessDays * 24 * 60 * 60 * 1000);
      db.prepare("UPDATE program_passes SET status='active', activated_token=?, activated_at=COALESCE(activated_at,?), expires_at=COALESCE(expires_at,?) WHERE code=?").run(token, now, expiresAt, code);
      return this.getPass(code);
    },
    async markInitialBuildCompleted(token, now) {
      db.prepare("UPDATE program_passes SET initial_build_completed_at=COALESCE(initial_build_completed_at,?) WHERE activated_token=?").run(now, token);
      return this.getPassForToken(token);
    },
    async incrementAdjustment(token) {
      db.prepare("UPDATE program_passes SET adjustment_count=adjustment_count+1 WHERE activated_token=?").run(token);
      return this.getPassForToken(token);
    },
    async successfulAdjustmentCount(token) {
      const pass = await this.getPassForToken(token);
    ...
    async expiredTokens(beforeMs) {
      return db.prepare("SELECT activated_token FROM program_passes WHERE status='active' AND activated_token IS NOT NULL AND expires_at < ?").all(Number(beforeMs)).map((r) => r.activated_token).filter(Boolean);
    },
  };
}
