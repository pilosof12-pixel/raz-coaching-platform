...
if (!q.includes('export function specificModalityRequirement')) {
...
if (!q.includes('SPORT-SPECIFICITY RULE:')) {
  q = replaceOnce(
    q,
    ...
    'modality prompt base rule'
  );
}

if (!q.includes('MODALITY-SPECIFIC HARD RULE:')) {
  const anchor = '  if (asksLowFatigueAerobicOnly(intake)) rules.push("AEROBIC HARD RULE: use 2-3 low-fatigue Zone 2 exposures, normally >=20 min for a meaningful dedicated exposure. No unrequested intervals, threshold, VO2, AMRAP or sprint conditioning.");';
  q = replaceOnce(q, anchor, anchor + '\n  const modalityReq = specificModalityRequirement(intake);\n  if (modalityReq && !modalityReq.externalSatisfied) rules.push(`MODALITY-SPECIFIC HARD RULE: ${modalityReq.key} is an explicit performance goal. Include at least one direct ${modalityReq.key} exposure in Week 1 and preserve modality-specific practice across the block. Cross-training may supplement it but cannot replace the named modality.`);', 'specific modality prompt');
}

if (!q.includes('STRENGTH-DAY RULE:')) {
  q = replaceOnce(
    q,
...
if (!q.includes('SPORT_MODALITY_SPECIFICITY_MISSING')) {
  const anchor = '  const primary = goalText(intake, "primary_goals"), secondary = goalText(intake, "secondary_goals"), all = `${primary} | ${secondary}`;';
...
if (!s.includes('ordinary lower-load strength sets may exceed a current rep benchmark')) {
  s = replaceOnce(
    s,
...
console.log(`${serverPath}: ${s !== originalServer ? 'patched' : 'already current'}`);
